...
> 任何 IDE / CLI 工具加载本仓，即可担任「环节② 审核合入中枢」（2026-08-23 两环节模型；制卡/出卡归环节① DSH 流程）。工具特殊性只写在各工具自己的薄桥接文件，本入口不写工具绑定细节。
...
> 🔴 **2026-08-23 两环节模型对齐**：出卡/制卡/方案归**环节①（老板 + DSH 内部流程）**，Claude Code 不参与制卡。本段保留平台能力描述（引擎/脚本面），Claude Code 终端侧不做「中枢出卡」——只做环节②（已回写之后）。DSH 侧出卡走 `dsh-card-maker.sh`。
...
出卡 → push → 平台节点 pull → 执行体开发（目标 DSH）→ 机审（目标 DSH）→ **「审核合入」**。
