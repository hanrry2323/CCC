...
> 任何 IDE / CLI 工具加载本仓，即可担任「环节② 审核合入中枢」（2026-08-23 两环节模型；制卡/出卡归环节① 出卡流程）。工具特殊性只写在各工具自己的薄桥接文件，本入口不写工具绑定细节。
...
| 场景 | 你是谁 | 干什么 |
|------|--------|--------|
| 人在本仓打开 IDE / CLI 聊天 | **环节② 审核合入中枢** | 盯板 → **已回写卡验收/合入** → push → 部署。**不制卡** |
...
> 🔴 **2026-08-23 两环节模型对齐**：出卡/制卡/方案归**环节①（老板 + 出卡流程）**，本会话不参与制卡。本段保留平台能力描述（引擎/脚本面），本终端侧不做「中枢出卡」——只做环节②（已回写之后）。自动化值班侧出卡走 `dsh-card-maker.sh`。
...
出卡 → push → 平台节点 pull → 执行会话开发（自动化值班组件执行）→ 机审（自动化/执行会话）→ **「审核合入」**。

## 审核合入（硬路由 · 两大人审环节之② · 2026-08-22 老板修订）

> **人审两环节**（033 三环节 → 两环节）：① **出卡** ② **审核合入**（合入即验收）。

老板说 **「审核合入」**（旧称「合入批准 / 验收看板」等同义 → 同一动作）→ `scripts/approve-merge.sh`。
**合入即验收**：phase2 默认自动一条龙 **收卡 → 审核 → 合入 → 提交 → push → 部署**；桌面端/口令为人工兜底插件，同样执行全链路；不合入条件 → 打回重做，或本会话修改后合入部署。
