...
> **叙事**：[`docs/VISION.md`](docs/VISION.md) · **事实+共识权威**：[`docs/product/loop-engineer-authority.md`](docs/product/loop-engineer-authority.md) · **版本**：`VERSION`（**v0.62.0**）  
...
**开发通道**：平台改动 = **仅 Cursor（不更换工具 · 完整 IDE 能力）**；Desktop 对话默认 Relay **`flash`**（→2017 `:4000`）。SSOT：[`docs/product/dev-channel.md`](docs/product/dev-channel.md)。  
