...
1. **收集上下文**：
   - `git diff HEAD~1 --stat` / `git diff HEAD~1` — 改动详情
   - plan.md 的 `## 验收清单` 段
2. **LLM 审查**：构造 prompt 喂 Claude（:4000, model flash），期望 verdict JSON
3. **判定**：
   - `pass` → move testing → verified（tester 随后跟进）
   - `fail` → 留 testing，记录 findings，dev 重试
   - LLM 不可用 → 按 R-12 分级（small→py_compile；medium/large→quarantine，**禁止静默 verified**）
4. **写报告**：`.ccc/reports/{tid}.review.md` 含完整 verdict + findings JSON

> verdict 必须写文件（红线 11：口头 PASS 不算 PASS）。report 不写 = 未审查。
...
- ❌ verdict 不写文件（必须写 `.ccc/reports/{tid}.review.md`）
