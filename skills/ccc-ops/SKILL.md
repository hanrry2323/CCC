...
5. **Relay 健康**（Stage 8 变更）：检查 M1 ai-loop-router（`http://192.168.3.140:4100`），不再检查本地 relay（2017 无 relay 实例）
6. **告警**：L1 (Info) 日志记录 / L2 (Warning) 桌面通知 / L3 (Critical) 写 `.ccc/alerts/` 存档
...
- `scripts/board/roles/ops.py` `ops_role()` — 入口（健康检查 + stale 检测 + 孤儿 PID 清理）
- `scripts/board/roles/common.py` `_quarantine()` — stale 检测到的异常任务移 abnormal
- `scripts/_ops_probe.py` — relay 健康探针（Stage 8：指向 M1 `:4100`）
- `scripts/ccc-relay-flash-watchdog.sh` — 2017 → M1 relay 看门狗（`http://192.168.3.140:4100`）
...
- **relay-端口变更** (Stage 8): relay 端口 4000/4002 → 4100/4102，2017 不再运行本地 relay 实例。Ops 的 relay 健康探针已更新指向 M1 `:4100`；`ccc-fleet.sh` 中 relay 相关逻辑已同步更新