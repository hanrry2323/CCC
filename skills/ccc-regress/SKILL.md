...
- **频率**: 每天 23:30（由 launchd com.ccc.regress 触发，硬编码在 install-ccc-roles.sh 的 23:30）
...
| 发现回归 → 建 `regression-<原task>` 到 backlog | 不改已发布的 tag |
...
for each released task:
  ...
  3. 每项跑一遍
     - 通过 → log "✓ task: 验收项"
     - 失败 → log "✗ task: 验收项 → 建 regression bug"
  4. 写回测报告
```
...
**通过标准**：所有验收项重新执行未发现回归。发现回归时已建 bug。
