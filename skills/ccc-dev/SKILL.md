...
1. 读 **工作区** `.ccc/state.md`（即 `<workspace>/.ccc/state.md`，相对 opencode `--dir`；
   **禁止**读 `~/.ccc/state.md` — 那是控制面，会被 external_directory 拒读）
   + plan.md + phases.json（同目录树：`<workspace>/.ccc/plans/`、`phases/`）
...
> Engine **不会**代写 `ALL SELF-CHECKS PASSED`；缺标记或日志出现 `external_directory` 拒读 → FAIL。
...
- ❌ 跳过工作区 `.ccc/state.md` 读取（红线 10）；❌ 读 home `~/.ccc/` 当看板
- ❌ 自检未全 PASS 退出（所有 6 条必须 PASS；门禁不代写）
...
- **假 PASS**：exit 0 + 拒读 `~/.ccc/*` 后门禁曾代写 `ALL SELF-CHECKS PASSED`（已禁；见 hollow-gate）
