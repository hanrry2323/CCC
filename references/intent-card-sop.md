...
| 角色 | 权责 |
|------|------|
| **人** | 唯一**发起**「转意图卡」；只审白话：要做什么、怎样算完；飞轮推到右栏的下一意图仍须人点转才进代办 |
| **Agent** | 人触发后起草**整条意图链**（多卡优先）；未收敛拒转；gate 红按 `fix_hint` 改卡 |
| **系统** | `transfer_gate` **仅绿**才 auto 进代办；空闲飞轮自动写下一 L1 `planned`（不写 backlog） |

**转意图卡 ≠ 定代办**。代办 = Engine 弹药；卡错则全错。

**一次点透 → 整条链（1）**：路线含 ≥2 个可独立验收变化 → 必须多块 `ccc-transfer` / `cards:[]`；禁止一轮一张糊大卡。  
