        ...
        for word in $cleaned_cards; do
          if [[ "$word" =~ ^[a-zA-Z]+-?[0-9]+ ]]; then
            ...
            if [ -n "$card_file" ] && [ -f "$card_file" ]; then
              ...
              # 状态提取 #6：sed 字符类只用 ASCII 空白（[[:space:]]），避免「已」第三字节
              # B7 与分隔符「·」尾字节 B7 碰撞把已* 状态截成单字节乱码；括号剥离用独立
              # 字面量表达式（s/（/、s/(/），不可用字节类 [（(]：「（」尾字节 0x88 仅与
              # 「分」（E5 88 86）的续字节碰撞即会把状态截断，派/关不含 0x88；
              # 并把「已作废」归一为「作废」，终态判定不再漏判。
              local c_status=$(echo "$card_head" | grep '状态：' | head -1 | sed -E 's/.*状态：([^[:space:]]+).*/\1/; s/·.*$//; s/（.*$//; s/\(.*$//; s/^已作废$/作废/' || true)
