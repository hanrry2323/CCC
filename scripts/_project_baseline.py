...
def baseline_prompt_for_claude(baseline: dict[str, Any]) -> str:
    ...
    return (
        ...
        "3. 验收命令是 Engine 关门条件，不是散文。"
        "板面残卡优先 `ccc-hub-lens.py repair <id> clear_blockers`，禁止默认投卫生 epic；"
        "偶发卫生卡才 executor=python + scope 仅 .ccc/。\n"
        ...
        "「可下达任务」给标题（≤20字）；编排产物脏优先 repair/提交产物，勿默认卫生 transfer。\n"
