#!/usr/bin/env bash
# qb 业务向 small（非 flow-smoke）：补 README 时间戳小节 → 无人值守至 done + commit 触达。
# 用法：CCC_SERVER=http://192.168.3.116:7777 bash scripts/smoke-qb-biz-small.sh
set -euo pipefail
...
TS=$(date +%s)
SUFFIX=$(printf '%x' "$$")
EPIC_ID="qb-biz-small-${TS}-${SUFFIX}"
CRID="qb-biz-${TS}-${SUFFIX}"
STAMP="qb-biz-${TS}"
export STAMP EPIC_ID PROJECT CRID

echo "== qb biz-small smoke against ${SERVER} epic=${EPIC_ID} =="
...
transfer() {
  ...
  while (( tries < 5 )); do
    if curl -sf --connect-timeout 8 --max-time 45 "${AUTH[@]}" \
      ...
      "${SERVER}/api/desktop/transfer" | tee /tmp/ccc-qb-biz-transfer.json \
      | python3 -c '
import json,sys
d=json.load(sys.stdin)
assert d.get("ok") and d.get("epic_id"), d
print("transfer ok", d["epic_id"])
open("/tmp/ccc-qb-biz-epic.txt","w").write(d["epic_id"])
'; then
      return 0
    fi
    tries=$((tries + 1))
...
transfer
EPIC=$(cat /tmp/ccc-qb-biz-epic.txt)
...
while (( SECONDS < deadline )); do
  if curl -sf --connect-timeout 8 "${AUTH[@]}" \
    ...
    >/tmp/ccc-qb-biz-snap.json 2>/dev/null; then
    eval "$(python3 - <<'PY'
import json
d=json.load(open("/tmp/ccc-qb-biz-snap.json"))
...
PY
    ...
    if [[ "${stage}" == "failed" || "${abnormal}" -gt 0 ]]; then
      echo "FAIL: stage=${stage}" >&2
      exit 1
    fi
    if [[ "${stage}" == "done" && ( "${released}" -gt 0 || "${verified_or_released}" -eq "${n_works}" ) ]]; then
      echo "pipeline done; verifying README commit on ${REMOTE}…"
      ssh -o ConnectTimeout=8 -o BatchMode=yes "${REMOTE}" \
        "cd '${QB_WS}' && git log -20 --oneline | grep -E '${EPIC}|${STAMP}' && grep -q '${STAMP}' README.md"
      echo "== qb biz-small PASS epic=${EPIC} stamp=${STAMP} =="
      exit 0
    fi
  fi
  sleep "${POLL}"
