...
# 6) flush outbox → transfer (Desktop flushTransferOutbox 等价)
BODY=$(python3 - <<'PY'
import json, os
from pathlib import Path
q=json.loads(Path(os.environ["OUTBOX"]).read_text(encoding="utf-8"))
crid=os.environ["CRID"]
item=next(x for x in q if x.get("client_request_id")==crid)
print(json.dumps({
  ...
  "complexity": item.get("complexity") or "small",
...
print("flush transfer ok", d["epic_id"], "idempotent", d.get("idempotent_replay"))
