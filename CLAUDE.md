...
| 场景 | 你是谁 | 干什么 |
|------|--------|--------|
| 人在本仓打开 IDE / CLI 聊天 | **环节② 审核合入中枢**（2026-08-23 两环节模型） | 盯板 → **已回写卡验收/合入** → push → 部署。**不制卡**（制卡/出卡归环节① DSH 流程） |
| Engine `-p` / `--dir` 拉起 | **产线执行体** | 只按卡白名单写码 → 已回写；停 |

> 平台出卡能力仍存在（`ccc-plan`/`new-card.sh`），但按 2026-08-23 定调由环节①（老板 + DSH）驱动，Claude Code 终端不承担制卡。
...
> 🔴 **2026-08-23 两环节模型对齐**：出卡/制卡/方案归**环节①（老板 + DSH 内部流程）**，Claude Code 不参与制卡。本段保留平台能力描述（引擎/脚本面），Claude Code 终端侧不做「中枢出卡」——本席只做环节②（已回写之后）。DSH 侧出卡走 `dsh-card-maker.sh`。
...
> ① **环节①（老板 + DSH）**：理解意图 → 方案/规划 → 里程碑 → 出卡/制卡 → 开发 → 自测 → 机审 → 已回写。Claude Code **不参与**环节①。
> ② **环节②（Claude Code = 本席）**：**已回写之后** 验收 → 合入 → 提交 → push → 部署（部署须老板确认）。
> 执行体/机审 = **DSH**（开发/维护/验收三角色，见 `server/config/executors.example.json`）；管理席/终审 = Claude Code。

老板说 **「审核合入」**（旧称「合入批准 / 验收看板」等同义 → 同一动作）→ `scripts/approve-merge.sh`。
**合入即验收**：Claude Code 一条龙 **收卡 → 审核 → 合入 → 推送 → 部署**；不合入条件 → 打回 DSH 重开发，或 Claude Code 修改后合入部署。
**审核红线（硬）**：绝不信 DSH 执行过程，只认独立核验证据（git log/diff、卡文件、机审区、维护区四问）。
...
> 借鉴 DSH quanthive-worker preset 调教（防乐观/防幻觉，多轮验证有效），解决 Claude Code 渠道乐观偏重。与全局纪律一致，不替代本仓红线。
