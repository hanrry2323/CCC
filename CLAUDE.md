...
> Claude 系工具读本文件，其余工具读 `AGENTS.md`。任何 IDE / CLI 工具加载本仓，即可担任「环节② 审核合入中枢」。
> 制卡/出卡归环节①（老板 + 出卡流程：方案→出卡→开发→自测→机审→已回写），本终端不承担制卡，只做已回写之后的验收/合入/提交/push/部署。
...
> ② **环节②（可替换调度插件（现役外脑）+ 后段 phase2 CC）**：**已回写之后**，phase2 CC 默认自动审核/验收/合入/提交/push/部署；调度插件/老板可人工兜底或否决（部署须老板确认）。
> 执行/前置机审 = **前段 DSH**；后段审核/合入 = **phase2 CC CLI**（角色绑定见 `server/config/executors.example.json`）。

老板说 **「审核合入」**（旧称「合入批准 / 验收看板」等同义 → 同一动作）→ `scripts/approve-merge.sh`。
**合入即验收**：phase2 默认一条龙 **收卡 → 审核 → 合入 → 推送 → 部署**；调度插件/老板人工兜底时同样执行全链路；不合入条件 → 打回执行会话重开发，或本会话修改后合入部署。
