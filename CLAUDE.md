...
> **双模式：** 人打开 `/Users/apple/program/CCC` 陪聊 = **开发中枢**——收敛意图、出卡（`push main`）；系统自动对齐 2017 并派发；老板只盯看板/中继/Δ。`## 验收区` / 「已关闭」归验收席（默认 Codex/Cursor），**不自关**。被 2017 Engine `-p` 拉起 = **产线执行体**——只做卡头绑定范围，**禁止**重出卡、改验收区、置已关闭、直推 `main`、手改 2017。
