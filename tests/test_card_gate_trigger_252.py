"""T-CCC-PLUGIN-01 序3：card_gate 触发条件集合化后，跨机执行体 PI@252 走五项校验。

验证点：
1. 执行体=PI@252 的合法卡 → 通过 gate（不再像单值等值时被静默跳过）。
2. 执行体=PI@252 的非法卡（缺实现要求 / 范围路径不存在）→ 拦截。
3. 执行体=DSH 老卡行为不变（老用例全绿，此处保一个门禁判据）。
4. 未受管执行体（CC@195 等）→ 仍直接放行。
5. 与 executors.json 槽位一致性：注册表含 PI@252。
...
def _write_registry(tmp_path: Path, include_pi195: bool = True) -> Path:
    """最小执行体注册表（含 PI@252 槽位时用 echo 占位，禁止生产引用）。"""
    executors = [
        {
            "角色": "开发执行体",
            "分类": "可后台 CLI",
...
def test_pi195_valid_card_passes_gate(gate_env: Path) -> None:
    """执行体=PI@252 的合法卡 → passed（此前单值等值会静默跳过，走不到五项校验）。"""
    card = gate_env / "tst986-valid-pi195.md"
    card.write_text(_card_text(executor="PI@252"), encoding="utf-8")
    work = _seed_work(card, "PI@252")
...
def test_pi195_valid_card_passes_five_checks_unit(gate_env: Path) -> None:
    """单元层：PI@252 合法卡经 validate_card 返回空问题清单（五项校验真实生效）。"""
    card = gate_env / "tst986-unit-valid.md"
    card.write_text(_card_text(executor="PI@252"), encoding="utf-8")
...
def test_pi195_missing_impl_section_is_rejected(gate_env: Path) -> None:
    """执行体=PI@252 的非法卡（缺实现要求）→ 拦截。"""
    card = gate_env / "tst982-missing-impl.md"
    card.write_text(_card_text(executor="PI@252", title="tst982 · 缺实现要求", with_impl=False), encoding="utf-8")
    work = _seed_work(card, "PI@252")
...
def test_pi195_scope_path_missing_is_rejected(gate_env: Path) -> None:
    """执行体=PI@252 的非法卡（范围路径不存在）→ 拦截。"""
    card = gate_env / "tst983-bad-scope.md"
    card.write_text(
        _card_text(executor="PI@252", title="tst983 · 范围路径不存在", scope="no/such/path.txt"),
        encoding="utf-8",
    )
    work = _seed_work(card, "PI@252")
...
def test_registry_contains_pi195_slot(tmp_path: Path) -> None:
    """executors.json 注册表含 PI@252 槽位（配套变更一致性）。"""
    reg_path = _write_registry(tmp_path, include_pi195=True)

    assert "PI@252" in _bindings(reg_path)
