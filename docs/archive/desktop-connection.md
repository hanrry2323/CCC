...
**热路径**：本机 Agent Sidecar（`127.0.0.1:7788`，**launchd `com.ccc.agent-sidecar` KeepAlive**）→ `vendor/loop-code/cli` → **M1 ai-loop-router** `:4100`（flash 档；可改本机 `relay.m1`）；fail-open → `CCC_RELAY_DIRECT_URL` / `~/.ccc/relay-direct.url`。  
Hub 默认：`http://127.0.0.1:17777`（`com.ccc.hub-tunnel`）。  
OpenCode（Engine 写码，Mac2017）：**ai-loop-router** `:4102`（code 档）；探活失败切直连兜底。  
