...
| **4000** | 旧 CCC Relay M1（已退役） | **M1** | 旧对话面模型路由（flash 档）；已迁移至 ai-loop-router :4100 |
...
| **4100** | ai-loop-router | M1 | Anthropic 协议，flash/pro 档；Engine 出口 |
| **4102** | ai-loop-router | M1 | openai-chat 协议，code 档；OpenCode 出口 |
...
（`#/chat` 挂在 Hub 上显示**内联跳转提示**（不再自动跳转），不是产品聊入口；对话走 M1 `:7788`。）
