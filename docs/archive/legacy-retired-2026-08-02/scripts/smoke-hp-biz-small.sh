#!/usr/bin/env bash
# hp 业务向 small（非 flow-smoke）：补/写 README 时间戳小节 → 无人值守至 done + commit 触达。
# 仿 scripts/smoke-qb-biz-small.sh（F3-1）。
# 用法：CCC_SERVER=http://192.168.3.116:7777 bash scripts/smoke-hp-biz-small.sh
set -euo pipefail
...
AUTH=(-u "${USER}:${PASS}")
PROJECT="${CCC_HP_SMOKE_PROJECT:-hp}"
WAIT_SEC="${CCC_HP_WAIT_SEC:-1200}"
POLL="${CCC_HP_POLL_SEC:-15}"
# shellcheck source=scripts/_smoke_remote.sh
source "$(dirname "$0")/_smoke_remote.sh"
REMOTE="${SMOKE_REMOTE_HOST}"
HP_WS="${CCC_HP_WS:-/Users/fan/program/apps/hp}"
TS=$(date +%s)
SUFFIX=$(printf '%x' "$$")
EPIC_ID="hp-biz-small-${TS}-${SUFFIX}"
CRID="hp-biz-${TS}-${SUFFIX}"
STAMP="hp-biz-${TS}"
export STAMP EPIC_ID PROJECT CRID

echo "== hp biz-small smoke against ${SERVER} epic=${EPIC_ID} =="
...
print(json.dumps({
  "project_id": project,
  "epic_id": epic,
  "thread_id": f"{project}::hp-biz-small",
  "client_request_id": crid,
  "title": f"新建并提交 README 双机路径备忘 stamp={stamp}",
  "goal": (
    f"hp 仓根目录缺 README.md（真实文档债）。"
    f"新建 README.md，写入一小节「CCC 双机路径（smoke {stamp}）」，"
  ...
  "plan_md": (
    f"# Plan — {epic}\n\n"
    f"## 目标\n新建 README.md 并写入双机路径备忘（stamp={stamp}）。\n\n"
    f"## 范围\n- **只改/新建文件**: `README.md`\n"
    f"- **禁止**: AGENTS.md、CLAUDE.md、.ccc/\n\n"
    f"## 步骤\n1. 若无 README.md 则新建；末尾追加小节，含 stamp={stamp}\n"
...
transfer() {
  ...
  while (( tries < 5 )); do
    if curl -sf --connect-timeout 8 --max-time 45 "${AUTH[@]}" \
      ...
      "${SERVER}/api/desktop/transfer" | tee /tmp/ccc-hp-biz-transfer.json \
      | python3 -c '
import json,sys
d=json.load(sys.stdin)
assert d.get("ok") and d.get("epic_id"), d
print("transfer ok", d["epic_id"])
open("/tmp/ccc-hp-biz-epic.txt","w").write(d["epic_id"])
'; then
      return 0
    fi
    tries=$((tries + 1))
...
transfer
EPIC=$(cat /tmp/ccc-hp-biz-epic.txt)
...
while (( SECONDS < deadline )); do
  if curl -sf --connect-timeout 8 "${AUTH[@]}" \
    ...
    >/tmp/ccc-hp-biz-snap.json 2>/dev/null; then
    eval "$(python3 - <<'PY'
import json
d=json.load(open("/tmp/ccc-hp-biz-snap.json"))
...
PY
    ...
    if [[ "${stage}" == "failed" || "${abnormal}" -gt 0 ]]; then
      ...
      cat /tmp/ccc-hp-biz-snap.json >&2 || true
      exit 1
    fi
    if [[ "${stage}" == "done" && ( "${released}" -gt 0 || "${verified_or_released}" -eq "${n_works}" ) ]]; then
      echo "pipeline done; verifying README commit on ${REMOTE}…"
      ssh -o ConnectTimeout=8 -o BatchMode=yes "${REMOTE}" \
        "cd '${HP_WS}' && git log -20 --oneline | grep -E '${EPIC}|${STAMP}' && grep -q '${STAMP}' README.md"
      echo "== hp biz-small PASS epic=${EPIC} stamp=${STAMP} =="
      exit 0
    fi
  fi
  sleep "${POLL}"
