...
> 项目：mx · 编号：mx-plan-008 · 状态：已完成 · 作者：Claude Code W1 · 工具：ccc-plan
...
- [x] 后端 core 模块在覆盖率排除配置缩减后，单元测试执行全绿通过。（d4f4c94 + workspace 578 全绿）
- [x] 后端实际测试行覆盖率（Line Coverage）不低于 80%。（CI 门禁 medio-core ≥80% 在位；v0.9.0 实测 80.09%→80.8%，当前值以 CI 为准）
