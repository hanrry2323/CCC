...
> 项目：cla · 编号：cla-plan-005 · 状态：已完成 · 作者：OpenCode · 工具：opencode
> 创建：2026-08-17 · 更新：2026-08-18
>  关联卡：已归档（原引用 cla019 随 8-24 治理归档，见 docs/archive 与 RETIRED 记录）
> 关联方案：cla-plan-004（原始数据源）
> 进度：1/1 (100%)
> 里程碑：M2 · 政府药械招采网价格监测
> 环境准备：Python >= 3.10, SQLite 3, Levenshtein 库（pip 可装）
...
- [x] 同一药品不同写法归一成功（单测覆盖 Levenshtein）
- [x] 重复抓取不产生重复行
- [x] 价格变化正确 upsert
- [x] 未命中 SSOT 的药名有「待人工确认」标记
