...
> 项目：cla · 编号：cla-plan-010 · 状态：已完成 · 作者：OpenCode · 工具：opencode
> 创建：2026-08-17 · 更新：2026-08-18
>  关联卡：已归档（原引用 cla021, cla022 随 8-24 治理归档，见 docs/archive 与 RETIRED 记录）
...
- [x] 三类机会判定单测（边界：=2 元不触发、库存正常不触发）
- [x] 机会幂等去重（同 raw_data 不重复）
- [x] 微信/短信/社媒三格式生成成功（mock LLM + 真实 ollama 各一）
- [x] 机会默认 pending_review 不直接推送
- [x] prompt 含合规约束提示
