...
> 项目：cla · 编号：cla-plan-003 · 状态：已完成 · 作者：OpenCode · 工具：opencode
> 创建：2026-08-17 · 更新：2026-08-18（实际已完成：eba676d 合入）
> 关联卡：无（实际由 eba676d 直接完成，未出卡）
> 关联方案：无
> 进度：0/1 (0%)
> 里程碑：M1 · 独立底座与路径清零
> 环境准备：Python >= 3.10（2017 运行面就绪）
> 子项目：1.3 债务收尾
...
- [x] dev-plan/OBS 全部移入 `docs/_archive/`，无原位残留
- [x] decided.json 目标/契约与架构定稿一致（diff 可复核）
- [x] 全仓无指向旧文件的活跃引用（grep 验证）
