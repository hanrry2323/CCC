...
> 创建：2026-08-20 · 更新：2026-09-08
> 关联卡：xy052、xy053、xy054、xy055、xy060、xy061、xy062、xy064、xy073、xy074、xy075
> 关联方案：无
> 进度：5/5 (100%)
> 里程碑：M6 · 前端展示台
...
- **范围**：仅激活 6.1「内容库 API」；实现只读内容库接口，扫描既有视频/图文产出并返回约定元数据。
- **依赖**：无代码级前置依赖；沿用现有 admin 只读适配层与既有产出目录，不扩展到生产核心链路。
- **验收边界**：只核对 `GET /api/v1/library` 的字段完整性、当日产出可见性和新任务自动收录；不包含工作流状态接口、预览页面或工作流可视化。
- **状态**：本次仅将 6.1 标为「计划中/已确认」并停在节点②前；6.2「工作流 API」、6.3「视频/图文预览页面」、6.4「工作流可视化页面」均未激活。
