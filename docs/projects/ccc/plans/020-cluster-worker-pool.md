...
> 项目：ccc · 编号：ccc-plan-020 · 状态：蓝图（待 252 实测验证后转执行计划） · 作者：OpenCode · 工具：OpenCode
...
| 4. 多通道模型 | 已确认可用（252 走 6100 flash） |
| 5. 集群并发扩展 | 待测 |
...
- [ ] 252 完成注册（W9），Engine 能按 W 号派发到 252
- [ ] 252 认领 ≥1 张开发卡并完成（绑定任务）
- [ ] 252 执行 ≥1 次巡检（非绑定任务）
- [ ] 中转站多通道在 252 上可配不同模型
- [ ] 2017 + 252 并发执行验证集群扩展
...
- 依托已落地：W 号寻址 v1（executors worker_id）、Loop Observer 框架（011）、中转站多通道
