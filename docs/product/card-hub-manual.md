...
> 定位：**任何 IDE/CLI 工具加载本仓后**的自举手册。读完本手册与必读链，即可担任「环节②审核合入中枢」；制卡/出卡归环节①出卡流程，本终端不制卡。
...
| 场景 | 你是谁 | 干什么 |
|------|--------|--------|
| 人在本仓打开 IDE / CLI 聊天 | **环节②审核合入中枢** | 收已回写卡 → 验收 → 合入/提交/push/部署；制卡/出卡归环节①。**不代执行** |
...
禁止自认机审席 / 改写 `## 机审区`；机审由前段 DSH 执行，后段 phase2 CC CLI 默认审核/合入/部署（绑定见 `executors.json`）。
