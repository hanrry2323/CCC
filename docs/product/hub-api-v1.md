...
> **原则**：产品主对话热路径在 **M1 Desktop sidecar**；Hub 做 transfer / flow / board / 健康，以及 **Remote Desktop 壳**（SPA + Agent 反代，见 [`hub-remote-management.md`](hub-remote-management.md)）。破坏性变更 → **v2**。
...
| `GET/POST` | `/api/desktop/threads` … | 会话镜像（非对话权威；Remote Shell 同步面） |
...
| `*` | `/api/agent/{path}` | **反代 M1 sidecar**（Remote Desktop 对话） |

产品主对话：**不**在 2017 跑第二套 Claude 槽。HTTP Remote Shell 经 `/api/agent/*` 打 M1 sidecar；thread id 与 Desktop 相同（`{project}::…`）。  
...
- Remote Shell：`GET /api/agent/health`（经反代）探测 M1 sidecar。

---

## 附录 A — Agent 反代（Remote Desktop）

> 契约全文：[`hub-remote-management.md`](hub-remote-management.md)。

| 方法 | Hub 路径 | 上游 |
|------|----------|------|
| `GET` | `/api/agent/health` | M1 `GET /health` |
| `POST` | `/api/agent/api/chat` | M1 `POST /api/chat`（SSE） |
| `POST` | `/api/agent/warm` | M1 `POST /warm` |
| `POST` | `/api/agent/api/session/drop` | M1 `POST /api/session/drop` |

Hub Basic Auth 后，反代注入 `CCC_AGENT_TOKEN`。浏览器不直连 sidecar。
...
5. Remote Desktop：`scripts/smoke-hub-remote-desktop.sh`  
6. 文档与实现字段一致（本文件 + transfer-gate）

---

*冻结日期：2026-07-20 · Remote Desktop 纠偏：2026-07-21 · 实现 PR 须引用 `hub-api-v1`。*
