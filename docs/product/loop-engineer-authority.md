...
| **PaidGuarantee（硬 · 2026-07-27）** | 免费池抖动时墙钟内**必试** Go 付费；默认 `FAILOVER_MAX_MS=35s` / `ATTEMPTS=6` / 单次上游 `LOOP_UPSTREAM_ATTEMPT_MS=15s`。`fetch`/超时**不**打 `provider_group` breaker；付费短冷却 ≤10s。旧坑：prio=80 垫底 + 120s 墙钟 → 付费排不到仍 503。见 `docs/relay/KEY-POOL.md` §503 |
