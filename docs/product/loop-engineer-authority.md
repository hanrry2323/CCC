...
| **PaidGuarantee（硬 · 2026-07-27）** | 免费池抖动时墙钟内**必试** Go 付费；默认 `FAILOVER_MAX_MS=60s` / `ATTEMPTS=6` / free `ATTEMPT_MS=20s` / paid `ATTEMPT_PAID_MS=55s` / peek free **12s** paid **25s**；墙钟 Abort 可打断进行中 fetch/peek。同请求最多 **2** 个 free。`fetch`/超时**不**打 `provider_group` breaker；付费短冷却 ≤10s。见 `docs/relay/KEY-POOL.md` §503 |
