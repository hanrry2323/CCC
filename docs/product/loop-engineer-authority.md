...
- **失败学习 R1/R2/R3（硬 · 2026-07-23）**：FAIL 打回前写 `.ccc/pids/{tid}.review_fail.md`；revert 后 phases 对齐；dev prompt 注入失败摘要。`review_fail_loops≥2` 或 plan_gap → **R2 修订该 work 的 plan**（禁止盲重试原指令；禁止 epic 子卡 product regen）。≥3 → R3 quarantine。本轮**不做** Ollama / 新 coding CLI。
