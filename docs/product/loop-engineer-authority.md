...
| **Agent 定卡培养闭环（硬 · 2026-07-29）** | 培养 Desktop Agent 定大卡/纠板，**禁止**靠 Cursor 反复救火当主路径。① **入学考试**：`transfer_gate` 拒弱探针/假绿/`plan` 无 `## 验收`/过宽 scope，返回 `fix_hint`；② **失败记忆**：`decided.transfer_lessons[]`（系统编译）+ digest「近期定卡教训」+ Engine `.ccc/lessons` 桥接；③ **处方改卡**：`failure_pack.exhausted[].optimize_hint` + `prior_transfer`；④ **qb 反模式**：`project_brain` / `transfer_playbook`。禁 invent / 禁抬重试 / 禁自动 stable |
