...
### 待综合方案（硬记 · 2026-07-22 压力矩阵实锤）

> **状态**：问题已登记；局部止血已上（reap / 埋点 / 主机曲线）。**完整方案另开一轮由 Cursor 给出并落地**，见 [`../briefs/2026-07-22-opencode-lifecycle-stall.md`](../briefs/2026-07-22-opencode-lifecycle-stall.md)。  
> **禁止**把下列症状再误诊成「倒卡太多 / 只加 MAX_CONCURRENT」。

| # | 症状（实锤） | 根因线索 | 局部已做 | 综合方案须覆盖 |
|---|--------------|----------|----------|----------------|
| A | 无 `opencode run` 进程，日志仍刷「同仓已有 active opencode」 | `engine-active-tasks` / 槽位认死 runner；`.done` 已落但未出 `active_tasks` | reap；人工清槽 | **done→收口→释槽** 原子化；死 pid 不得占同仓互斥 |
| B | `.done`+exit 0 仍卡 `in_progress` | `result.json` 被 `opencode-exec` 日志污染成非法 JSON；recover 走 relaunch 而非 success | 手工修 JSON / 挪列 | runner **只写纯 JSON**（日志另文件）；check_complete 容忍/剥离前缀 |
| C | 卫生卡 `executor=python` 仍进 opencode 长跑 | board_ops 短路径未稳定抢先或失败后 fallback | board_ops 存在 | 短路径失败不得默默改走 opencode；失败即明确门禁 |
| D | Engine CPU 0%、planned 全延后 | testing 门禁同步等 `claude -p`，reviewer 挂死堵整 tick | 杀 claude / 清 lock | testing **不堵** launch 环；reviewer 超时/低 CPU 必杀+跳过 |
| E | planned+testing 双文件；verdict FAIL+revert 冲突 | 列移动与 recover 竞态；工作区脏挡 revert | 删重复文件 | 单卡单列不变量；revert 前 stash/隔离 `.ccc` 脏 |
| F | 「能否加并行」无据 | 缺主机曲线 | `host-resources.jsonl` + CLI | 用 p95 load/mem + 卡死率决定 `MAX_CONCURRENT`，**先治 A–D 再加并行** |

**综合方案验收（预告）**：连续压测同仓多卡时——无幽灵「同仓延后」、无脏 `result.json` 卡死、卫生卡不进 opencode、reviewer 挂不死 Engine、小卡 `wall_min` 可统计。
