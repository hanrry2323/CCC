...
#### Desktop 板务白名单（硬 · 2026-07-24）

对话面可经 Hub **`POST /api/desktop/board-repair`**（CLI：`ccc-hub-lens.py repair`）修板，**绝不**写业务源码：
...
| 审计日志落盘 | 用 Engine 卫生 epic 当清场主路径 |

未 ready：Agent **先 board-repair**；仅业务脏 / 真在飞冲突时拦新产品 epic，并说明人可「仍要下达」（显式 override，记 `human_note`）。
