...
**Agent 定方案（硬）**：讨论 / 定稿时，按用户意图制定**最佳方案**并默认推进；**禁止**每轮甩拍板选择题、**禁止定稿后再问要不要入队**。仅当真缺不可逆信息才最多 1 问。定稿白话给用户可读结论；`ccc-transfer` 契约折叠给 Engine，不把任务 id/路径清单当结论。**板面残卡清场**归 **当前 Desktop App Agent**（`hub_repair` / board-repair，含孤儿 running），**禁止**默认逼用户再投卫生 epic，**禁止**甩锅「请打开编排运维」。偶发仍投卫生卡时：`executor_intent=python`；**不存在 committer 绕过 Engine**；`pipeline=ops` 仍扇出。abnormal / 未核账在飞残卡禁止重复下达同目标卡（须先本会话清板，或人显式 override）。
...
| 对齐基线 | 可点；**非硬门槛** | Hub baseline + live lens；扫出残卡 → **本会话 `hub_repair` 清板**，不默认逼卫生 transfer |
| 看仓况 | 可选芯片（旧名「下一步」已降级） | lens `board`+`git`；阻塞则自清；**不是**下达必经阶段 |
...
- **hang 收尸让下一卡（硬 · 2026-07-24 · 方案 A）**：无进展默认 **300s**（`CCC_PHASE_NO_PROGRESS_SEC`）；CPU hang 检查间隔 **120s**；同卡 hang 重试 ≤**1**。stash/kill/relaunch 失败也必 orphan-reap + 释槽（禁止删 `.hung` 却占槽）；耗尽 quarantine 后再 reap 一次。释槽后**同 tick**优先 `_try_launch_planned` 该仓下一 planned。仍同仓 1 路 OpenCode，不加 `MAX_CONCURRENT`。
- **seed 扇出快路径（硬 · 2026-07-24）**：定稿已挂 plan+phases 时 `fanout_from_seeded_epic` 跳过 Claude；child `## 验收` **禁止散文**，须白名单可重放探针（`test -f` / `py_compile` / DRY_RUN…）。`plan_lint` 失败先本地修验收再试 1 次，仍失败才回退 Claude。已拆 work 全 planned 且同仓有 in_progress → flow `queue_hint=same_ws_opencode`（排队 ≠ 扇出卡住）。
...
| **PaidGuarantee（硬 · 2026-07-28）** | 免费池抖动时墙钟内**必试** Go 付费；默认 `FAILOVER_MAX_MS=45s` / free attempt **8s** / paid **25s** / peek 6s·12s；**仅 zero 可用 free** 才 paid-first；日配额只冷却该钥（不封出口）；**会话钉 paid**（成功走 Go 后 `pinPaid` TTL≈24h 保 prompt cache，亲和键=`x-session-id` 或 system+**首条**user，禁止 last-2-user；`:4000`/`:4002` 均打 `prompt_cache_*`；fallback 尊重候选序禁同会话双付费 RR；≠背叛新会话 free-first）。主 KPI=`upstream_cache_token_ratio=cached/prompt`（勿把 L1/`cache_hit_ratio` 当账单）。见 `docs/relay/KEY-POOL.md` |
