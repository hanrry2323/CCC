...
| `recent_lessons` | 未 fixed 失败教训 | `_lessons.get_recent_lessons` |
| `success_lessons` | 同主题成功经验（F4-2） | `_lessons.get_lessons_by_topic`（kb `record_success` → `.ccc/lessons/<topic>.md`；product 按 title/tag 主题注入「## 同主题经验」） |
...
| 角色 | keys | 迁移状态 |
|------|------|----------|
| `product` | skill, baseline, profile, code_ctx, ref_plans, recent_lessons, current_epic, plan_template（另：`success_lessons` 由 product 按主题注入，见 F4-2） | **已迁** |
...
| `regress` | plan, recent_lessons, current_epic | TODO（占位；执行路径已用 `_intent_probe` 重放验收，见 v0.60） |
