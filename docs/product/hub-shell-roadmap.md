...
> **状态**：已拍板（2026-07-20）· 对齐版本 `VERSION`（**v0.60.0**；壳线收口于 v0.52.2，意图飞轮见 [`lpsn-ship-gate.md`](lpsn-ship-gate.md)）
...
**止损最小可见（Wave3 Phase9）已勾**：Desktop 右栏红条 + toast + 链到运维/看板；Hub snapshot `user_stage=failed`（含 epic `split_status=failed`）。仍不引入逐步人批。
...
- 在 Mac2017 再维护**第二套**产品聊天 runtime，或把对话 SPA 挂在 Hub `:7777` 再反代 sidecar（双口：对话=M1 / 编排=2017，见 [`hub-remote-management.md`](hub-remote-management.md)）  
...
聊天热路径：**M1 Desktop sidecar（loop-code）**，客户端直连 `:7788`。Hub 只做编排 API；不在 2017 另起聊天槽，也不以 Hub `/api/agent` 为产品主路径。

完整字段表：[`transfer-gate.md`](transfer-gate.md) · API 契约：[`hub-api-v1.md`](hub-api-v1.md) · 双口远程：[`hub-remote-management.md`](hub-remote-management.md)。
...
**Phase6 已勾选**：[`hub-shell-phase6-qb.md`](hub-shell-phase6-qb.md) — `qb` 一笔 small 至 released。  
**Phase8 已勾选**：[`hub-shell-phase8-hp.md`](hub-shell-phase8-hp.md) — 第二真实仓 `hp`（xianyu 脏跳过）至 released。  
**Phase9 已勾选**：[`hub-shell-phase9-stoploss.md`](hub-shell-phase9-stoploss.md) — abnormal 止损最小可见。  
...
**Phase15 已勾选**：[`hub-shell-phase15-flow-rail-cards.md`](hub-shell-phase15-flow-rail-cards.md) — Desktop 右栏卡片内容（UX 阶段表全档主文案 + goal/executor/depends_on_titles/failure_note/note 上卡）+ 视觉层次（running/failed/done 区分 + reveal 闪烁收敛）。  
**Phase16 已勾选**：[`hub-shell-phase16-cold-start.md`](hub-shell-phase16-cold-start.md) — Desktop 本地优先冷启动（磁盘秒开，Hub 后台同步）。  
**Wave A / Phase17 已勾选**：[`hub-shell-wave-a-lan.md`](hub-shell-wave-a-lan.md) · [`hub-shell-phase17-model-picker.md`](hub-shell-phase17-model-picker.md) — Hub LAN + Desktop 模型快选（v0.52.2）。

---

## 9. 文档与仓库卫生清单

- [x] 本文为下阶段北星；重大转向先改本文再改代码  
- [x] `CHANGELOG` / `VERSION` 与发布说明同步（**v0.52.0** · [`../releases/v0.52.0.md`](../releases/v0.52.0.md)）  
- [x] 新目录仅项目根下一级；inbox = `CCC/inbox/`（禁止 `.ccc/inbox` 双轨）  
- [x] Mac2017 / M1 部署 commit 对齐后再宣称「已上线」（Wave3 Phase7）  
- [x] 契约变更：先改 docs，再改 Hub，再改 Desktop  
...
Wave4 Phase10–12 已绿。**v0.52.1 稳定性门禁**已落地。**Phase14–16** 右栏线已绿（绑定 / 卡片 / 本地优先冷启动）。**Wave A** Hub LAN 可达已验收（[`hub-shell-wave-a-lan.md`](hub-shell-wave-a-lan.md)）。**Phase17** Desktop 模型快选已绿（[`hub-shell-phase17-model-picker.md`](hub-shell-phase17-model-picker.md)）· **v0.52.2**。
