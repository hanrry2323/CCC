...
- [ ] 新目录仅项目根下一级；inbox 若建则为 `inbox/`（或 `.ccc/inbox/` 若需藏数据 —— **二选一写进实现 PR，禁止两处并存**）  
...
1. 开实现计划（可拆 PR）：API v1 文档补全 → Desktop 三态 → 可靠性 P0  
2. 代码变更必须引用本文章节号（如 `hub-shell-roadmap §6 P0`）  
3. **未点名开干前**：只维护文档与契约，不铺旁路业务代码  
