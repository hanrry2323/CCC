...
终态架构：**任意设备壳**（HTTP 直连）+ **2017 单端 :7788**（薄驱动 Engine + 文档流转 + 看板/HTTP）+ **执行体**（经注册表派发：执行会话 + 自动化值班组件，模型档 flash vs code）。
...
完整见 `references/red-lines.md`。CCC 红线集（v2.0.0 现行）：
- 18 条编号红线（核心安全/契约/边界）
