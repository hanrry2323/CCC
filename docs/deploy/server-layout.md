...
- **生产执行、Hub/Engine** 以 2017 为准（模型经 M1 ai-loop-router 路由，三档 flash/Pro/code；fail-open → `CCC_RELAY_DIRECT_URL` / `~/.ccc/relay-direct.url`）  
- **M1 对话**：Desktop + sidecar `:7788` + arm64 `vendor/loop-code/cli` → **M1 ai-loop-router** `:4100`（主，免费 flash；可改本机 `relay.m1`）→ fail-open 直连文件  
