...
## Lesson 48：test-events-001 进入异常状态

**项目**：`/Users/apple/program/CCC` | **Phase**：N/A | **时间**：2026-07-16 11:05:42 UTC
...
## Lesson 50：机审 infra 判定过宽会吞掉真实的「机审：不通过」

**问题**：hp003 卡在机审 7 轮空转。审计日志出现 `timeout` 字样被 infra 判定吞掉，
真实的「机审：不通过」（3 项 P1）从未回待分派给执行体 → 卡留「已回写」反复重审。

**根因**：`is_retryable_failure` 对日志关键词扫描过宽（裸 `timeout` 也算 infra），
且 infra 路径没有次数上限——业务结论与基础设施故障混在同一判定里。

**修复（2026-08-07）**：业务结论优先（审计明确「不通过」绝不被弱特征误判）；
infra 关键词收窄（仅 503/上游不可用/网络错误等强特征）；infra 连续 3 次回待分派人工跟进；
审计独立超时 `EXECUTOR_AUDIT_TIMEOUT_SECONDS`（默认 30 分钟）。

**如何应用**：任何「自动重试」路径都必须区分业务失败与基础设施故障，且给自动重试设上限；
LLM 输出里的弱关键词（timeout 等）不能作为故障归因的唯一依据。

## Lesson 51：机审多轮结论不能被「首个锚点后 20 行」窗口误判

**问题**：xy012 多轮机审在同一节内追加多轮结论（历史不通过 + 通过），
通过结论在首个锚点 20 行之外 → 整卡被误判未通过 → approve-merge 拒绝 + 引擎反复重审。

**根因**：`machine_audit_passed_text` 只认首个 `## 机审区` 锚点后 20 行。

**修复（2026-08-07）**：改为逐节扫描至下一节标题（无行数上限），任一「通过」即通过；
历史「不通过」轮不构成否决。

**如何应用**：对会追加历史记录的文档结构做判定时，判定必须遍历全部节，不能假设单节单结论。

## Lesson 52：看板高 CPU 的根因是轮询 + 逐卡富化，不是引擎

**问题**：2017「engine」CPU 高。实测 engine 仅 0.4%（睡眠等待）；
真凶是 web-server——Edge 看板页两套 5s 轮询，每次 `/cards` 全量扫目录 + 逐卡日志/工作树计算。

**根因**：读文件/跑 git 的成本按请求 × 卡片数放大，缓存 TTL（3.5s）小于构建耗时（6s）导致缓存永不命中。

**修复（2026-08-07）**：`/cards` 合成视图 + 整表富化缓存（TTL 10s、运行 marker 变化才失效）；
富化收窄到非关闭卡 + 最新 10 张已关闭；轮询命中 4ms、重建 1.3s。

**如何应用**：服务端聚合页必须整表缓存（TTL > 构建耗时）；逐卡子进程/日志计算要按活跃集收窄。

## Lesson 53：审计挂起会长期占槽，需要独立超时与可见状态

**问题**：机审超时配置沿用执行超时 7200s，Claude 挂起时审计槽一占 2 小时，
后续卡排队；且机审列无状态提示，卡「不动」时看不出在审核/冷却/待审。

**根因**：审计与执行共用超时；机审列只有列归属没有过程状态。

**修复（2026-08-07）**：`EXECUTOR_AUDIT_TIMEOUT_SECONDS=1800` 独立审计超时；
...
## Lesson 55：卡编号即派发现场——出卡后禁止改名/重编号（clw006 事故）

**问题**：clw006 打包验收卡反复打转死循环（空回写 → 机审打回 → engine 无限重试），卡在流程里打转数小时不可观测，直到人工介入。

**根因**：修复卡自动编号占用了 clw006 后，被改名 clw007；但 engine 的 worktree/runtime/card_path 仍残留旧路径 `clw006`，执行体按旧 card_path 读不到卡 → 空回写 → 机审打回 → 无限重试（且可能被误判为 infra 冷却）。

**修复（2026-08-10）**：
- 平台层（ccc039）：方案链编号保护（编号被方案占用必须显式 `--id`）+ `work.id → card_path` 每次从磁盘索引重新匹配（`store.py`）+ 空回写直接打回不再无限重试（`main.py is_empty_writeback_or_placeholder`）+ infra 连续失败熔断打回。
- 流程层：出卡后不改名/改 ID；附加卡/修复卡用 `--id` 显式编号；改名必须同步清理 worktree + runtime + 远端分支。

**如何应用**：发现「卡打转 / 空回写 / 无限重试」先查是否发生过改名或编号冲突；出卡时编号被占先 `--id`，绝不自作聪明改名；平台侧修复已由 ccc039 落地，后续遇到直接对照根因链（编号 → card_path 残留 → 空回写 → 重试上限）。

## Lesson 56：业务仓必须有每卡隔离——worktree 缺失 = 主仓互踩事故（mx030-034 事故）

**问题**：mx030-034 五张卡投入 medio-0 业务仓，在同一主目录反复 checkout/reset 互相覆盖，工作区污染，五张卡被紧急删除（2026-08-12）。

**根因**：engine 的 worktree 隔离只覆盖 CCC 仓（卡回写域），业务仓型任务（registry 指向外部仓）没有每卡 worktree；出卡模板又硬编码业务仓路径，引导执行体在主仓目录 checkout 卡分支 → 多卡并发/串行共享同一可变区。worktree 创建失败时还会静默回退默认工作目录，把隔离失效藏起来。

**修复（2026-08-12 隔离升级）**：
- registry 每项目 `isolation.worktree_root` + `max_concurrent`（默认 1），业务仓型任务派发时在业务仓建每卡 worktree，执行体 `--dir` 指向业务仓 worktree。
- 删除「回退默认工作目录」路径：worktree 失败 = 基础设施冷却，卡保持待分派，绝不静默降级。
- 同业务仓并发闸门（默认 1），跨仓可并行。
- 出卡模板不再硬编码业务仓路径，改为「Engine 派发注入 worktree」。
