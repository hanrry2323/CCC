...
> 本区为机审打回后的第 5 次回写（本轮收卡状态「待分派（机审打回·重试中）」，打回信封 `68674d12d` 仅翻转状态行、未附原因/批注，人工批注节为空）；第 5 轮对既有实现全量复核（文件逐字比对＋门禁复跑＋远端同步与祖先关系核验），业务仓零代码改动。
