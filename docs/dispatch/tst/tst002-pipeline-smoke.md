# 任务卡 tst002 · 本卡末尾追加独立行 tst-001-smoke 并回写（DSH 执行）

> 关联：tst-plan-001 · 执行体：DSH · 验收：DSH · 状态：作废（冒烟卡使命完成（验证全链路+暴露机审维护区假断言），维护区未填无法合入，老板批准） · 派发：engine · 项目：tst · 日期：2026-08-23
...
本卡末尾追加独立行 tst-001-smoke 并回写（ccc-plan 切片）。
...
- `本卡文件自身（docs/dispatch/tst/ 下本卡 .md）；禁止改任何其他文件`
...
1. 本卡文件内存在且仅存在一行整行精确匹配的冒烟标记行（grep -cx = 1，标记串见目标），且卡头状态=已回写
2. 回写区含 commit hash 与 push 证据；维护区四问逐项真实填写
...
- 关联方案摘要：目标：验证「真实看板出卡 → Engine 全自动派发 → DSH 开发执行 → 机审」链路在 tst 自检项目上可用；改动仅落在任务卡文件自身。验收标准：tst001 卡经 Engine 自动派发后由执行体回写，卡头状态=已回写 卡文件存在且仅存在一行整行精确匹配的冒烟标记行（`grep -cx` = 1） 机审通过并写入「## 机审区」，台账有 machine_audit_pass 记录。
