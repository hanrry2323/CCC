# 任务卡 tst902 · smoke: card flow full-probe（DSH 执行）

> 关联：阶段 3 P1 · 执行体：DSH · 验收：DSH · 状态：作废（死循环探针卡，熔断处置，永久停用） · 派发：engine · 项目：tst · 日期：2026-09-03
...
docs/dispatch/tst/tst902-smoke-full-probe.md

## 步骤

1. 在回写区第 0 行写入本卡标题全文「tst902 · smoke: card flow full-probe（DSH 执行）」——证明执行体已读到本卡内容。
...
1. 回写区第 0 行含本卡标题复述「tst902 · smoke: card flow full-probe」。
