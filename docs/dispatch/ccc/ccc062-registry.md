# 任务卡 ccc062 · registry单真值收敛与方案卡同步闭环（OpenCode 执行）
...
registry 单真值收敛：seed 兜底改只读归档、executors.json 项目行并入 registry 派生、解析器统一；卡关闭时自动写方案关联卡 + Q1 收紧为 AND。

## 红线（先看）

1. 禁止改动 registry.yaml 结构本身（只改消费方）；项目注册仍只改 registry
2. 若本卡含 `## 人工批注`，执行体必须先读批注并按批注修订目标/步骤后再执行；批注优先于正文。
2. 若本卡含 `## 人工批注`，执行体必须先读批注并按批注修订目标/步骤后再执行；批注优先于正文。

## 范围

- server/web/server.py（_PROJECT_METADATA_PATH seed 兜底）
- server/board/registry.py、scripts/validate-plans.sh（解析器统一）
- server/engine/ 或 scripts/approve-merge.sh（方案关联卡写入钩子）
- server/board/plans.py（Q1 OR→AND）
- server/tests/**

## 步骤

1. seed 兜底改只读归档（GET /projects 仅 registry；seed 仅历史展示）
2. executors.json 项目级绑定改由 registry 派生（仓外文件只留执行体命令参数）
3. validate-plans.sh 的 grep 解析换用 server/board/registry.py
4. 卡关闭钩子（approve-merge.sh）自动调 plans 写关联卡 + 推进方案状态；Q1 校验收紧 AND
5. 测试套件 + 全量 validate；commit+push 到卡内分支（勿直推 main）；卡头改为「已回写」。
...
1. GET /projects 输出与 registry 完全一致（seed 不再补充）
2. 新注册项目无需手改 executors.json 即可派发
3. 关闭一张测试卡后方案「关联卡」自动更新；Q1 单条件不再通过
