# 任务卡 ccc061 · 告警推送与watchdog挂载（OpenCode 执行）
...
补上零告警缺口：board-live.sh 探活失败推送 macOS 通知；watchdog-ccc.sh 挂 2017 launchd 每 60s 探服务心跳。

## 红线（先看）

1. 不新增服务端口；只用现有 launchd/通知机制
2. 若本卡含 `## 人工批注`，执行体必须先读批注并按批注修订目标/步骤后再执行；批注优先于正文。
2. 若本卡含 `## 人工批注`，执行体必须先读批注并按批注修订目标/步骤后再执行；批注优先于正文。

## 范围

- qx-map/sync/board-live.sh（失败通知）
- qx-map/sync/（新增 watchdog plist 或 2017 侧挂载）
- 2017 `scripts/watchdog-ccc.sh`（launchd 挂载）
- ~/Library/LaunchAgents 或 2017 LaunchAgents

## 步骤

1. board-live.sh 探活失败分支加 osascript 通知（M1）
2. watchdog-ccc.sh 挂 2017 launchd（每 60s 探 engine/web-server/board-scheduler/ai-loop-router 心跳，失败写 ~/.ccc/alerts/ 并尝试告警）
3. 实测：停一个服务观察告警产生（测试后恢复）
...
1. launchctl list 含 watchdog，60s 间隔生效
2. 模拟服务故障：alerts/ 产生新文件 + M1 收到通知
3. 恢复后无残留告警
