# 任务卡 ccc060 · 模型出口多上游兜底恢复（OpenCode 执行）
...
模型出口每档恢复 ≥2 个 enabled 上游：恢复 code 档 free-1..5 与 flash 档备用上游，按 tier_priority 排序，立「上游变更必须留热备」规则。

## 红线（先看）

1. 动 2017 生产配置前必须先备份 upstreams.json 与相关 plist
2. 若本卡含 `## 人工批注`，执行体必须先读批注并按批注修订目标/步骤后再执行；批注优先于正文。
2. 若本卡含 `## 人工批注`，执行体必须先读批注并按批注修订目标/步骤后再执行；批注优先于正文。

## 范围

- 2017 `apps/ai-loop-router-ccc/upstreams.json`（生产配置，SSH 只读备份后改）
- `~/.ccc/relay/upstreams.json`（双写密钥表同步核查）
- 探活：`EXECUTOR_PROBE_URL` 6100 与 6102 双端口
- 文档：qx-map AGENTS.md 模型出口段（档位门禁规则）

## 步骤

1. 备份 2017 upstreams.json（带日期后缀）
2. 恢复 free-1..5 enabled、按 tier_priority 排序；flash 档留 ≥2 上游
3. 验证：code/flash 各实测一次 HTTP 200；断流模拟（禁用主上游）实测兜底生效
4. 双写密钥表（relay/upstreams.json）与主表对齐，消除两处配置差异
...
1. upstreams.json 每档 ≥2 enabled，有备份文件
2. 实测 code/flash HTTP 200；禁主上游后兜底上游接管成功
3. 双写密钥表一致
