...
**判定：✅ 通过。** 附 1 项质量观察（多套教训编号易混淆，已并入 T38 验收用例）。

### 对照承诺表

| 验收标准 | 实际 | 判定 |
|----------|------|------|
| 1. 命中注入有单测覆盖；降级路径（未配置/未命中/异常）不报错 | 15 例单测覆盖命中注入/未命中/未配置/异常/非知识/不回归，断言真实（prompt 含参考块、降级不注入）；实测 295 collected 0 失败 | ✅ 做到 |
| 2. 5 题知识问答引用命中；非知识不劣化 | 执行体实测 6 题（5 知识 + 23×47=1081）全 200 且引用 KB；Codex 独立复测 1 题（LC1）经 2017:6100 真实大脑 200 且引用知识库内容作答 | ✅ 做到（编号混淆见观察） |
| 3. pytest 全绿；三扫描零命中 | 295 collected 0 失败；ruff server/ All checks passed；硬编码/密钥/外脑引用零命中（qx-map/hp-kb 仅出现在红线禁止语句） | ✅ 做到 |
| 4. 真实提交；工作树干净；push 自证（红线 5） | 468c260+aae0664 已 push（origin/main 实测 = aae0664）；工作树干净 | ✅ 做到 |

### 质量观察（并入 T38 验收）

- **教训编号混淆**：Codex 独立实测「LC1 是什么」→ 大脑引用了知识库真实内容但返回的是 L1（Plan 用自然语言）的标题，与 LC1（文档口径分裂）混淆。检索注入本身工作正常（答案引用 KB 真实条目），属多套编号（L/LC/EB-L）下模型精度问题。T38 生产验收加一条「编号歧义用例」（LC1 vs L1 同时可检索时回答须区分），并可在注入段落显式携带条目 id。
- **首次独立实测 120s 超时**：claude 自行工具绕行（翻文件）导致超时；引导「只看参考段落」后 14s 正常返回。T38 部署时确认生产 CCC_BRAIN_TIMEOUT ≥120（建议 180）并保持 prompt 引导语。
