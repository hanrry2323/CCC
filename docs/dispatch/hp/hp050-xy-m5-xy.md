# 任务卡 hp050 · xy 接入（M5） — 实施「xy 接入」（OpenCode 执行）

> 关联：hp-plan-027 · 执行体：OpenCode · 验收：OpenCode · 状态：已关闭· 派发：engine · 项目：hp · 日期：2026-08-17
...
- 关联方案摘要：目标：xianyu 建立知识域（现 0 docs），项目知识/决策/教训入 HP。验收标准：xy 接入完成，验收点可复核（命令/可观察结果）。
