...
> 关联：ccc-plan: HP 知识底座落地推进（存量落库/采集管道固化/qb 归属修正） · 执行体：OpenCode · 验收：OpenCode · 状态：打回（机审未通过） · 派发：engine · 项目：hp · 日期：2026-08-08
...
- P1 证据/追溯性缺陷三件未闭环（88 篇文档恢复无提交式 restore 脚本且机制误述；documents.total=4356≠4278 校验被重定义且未如实披露并发增量；业务提交 author 不符验收#5）。
- 需：
  1. 为 88 篇恢复补一份可复现的提交式 restore 脚本/SQL + 回滚路径（user=hp@local），修正回写区机制描述。
  2. 披露 documents.total 实值与并发增量，将 verify 恢复到真实总数口径门禁（禁止用子集代替总数）。
