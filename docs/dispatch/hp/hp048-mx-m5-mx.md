# 任务卡 hp048 · mx 接入（M5） — 实施「mx 接入」（OpenCode 执行）
> 批准：老板合入批准 · 2026-08-17

> 关联：hp-plan-025 · 执行体：OpenCode · 验收：OpenCode · 状态：已关闭· 派发：engine · 项目：hp · 日期：2026-08-17
...
- 关联方案摘要：目标：medio-0 方案/教训/决策回流 HP（建立 mx 域），开发时检索历史方案/教训。验收标准：mx 接入完成，验收点可复核（命令/可观察结果）。
