# 任务卡 hp033 · 运行时↔SSOT 对齐（M2） — 实施「运行时↔SSOT 对齐」（OpenCode 执行）

> 关联：hp-plan-010 · 执行体：OpenCode · 验收：OpenCode · 状态：已关闭· 派发：engine · 项目：hp · 日期：2026-08-17
...
- 关联方案摘要：目标：运行时 mcp_server（含 kb_status）回灌 SSOT；SSOT 领先部分回灌运行时；diff 确认两端一致。验收标准：运行时↔SSOT 对齐完成，验收点可复核（命令/可观察结果）。
