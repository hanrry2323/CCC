...
**判定：✅ 通过。** 附 1 个流程修正项（P2-4：交付时未推送，验收席已补推）+ 2 项观察。

### 对照承诺表

| 验收标准 | 实际 | 判定 |
|----------|------|------|
| 1. 四类种子 updated_at=2026-08-03；新增决策 ≥6、教训 ≥4；来源可追溯；无敏感信息 | 实测：四类 updated_at=2026-08-03；决策 11 条（6 新增：重构 v2/D11 双轨/收口重评/T31–T35/M2/D10 细则）；教训 9 条含 LC1–LC4；每条决策带 `file` 主档指针；密钥/密码/token 扫描零命中 | ✅ 做到 |
| 2. 索引重建成功；5 查询词命中对应域；pytest 全绿 | Codex 独立复测：索引 80 文档；5 词实测全命中（2017 单端→nodes-paths / 中转站双轨→decisions / 文档口径分裂→lessons / QuantHive→projects / D10→decisions）；pytest 280 collected 0 失败 | ✅ 做到 |
| 3. 退役端口/旧架构表述零残留（现行文档） | 退役端口仅存在于 nodes-paths「已退役端口（权威清单）」——每行带 T31/T34 归档追溯与现状说明（如 qx-observer 仍运行），为知识性登记非陈旧表述；测试 `test_retired_ports_*` 显式允许并校验上下文 | ✅ 做到（含上下文登记，优于裸注释） |
| 4. 工作树仅剩许可预存项；真实提交 | 工作树全干净；a466f32+d12f35f 真实提交（**交付时未推送，验收席补推 0c7e8cc..d12f35f**） | ⚠️ 提交做到，推送由验收席补 |

### 流程修正项 P2-4（生效于后续卡）

- 交付回写前必须 `git push origin main` 成功，并在回写区附 push 输出或重试说明（T36 教训：commit 未推送，origin 停留旧 HEAD，2017/看板未同步）。已写入 T37/T38 红线。

### 观察（非阻塞）

- 决策条目 `file` 为主档文件名（符合 D2 独立口径，来源追溯元数据不构成运行时读外脑）；T38 M4 落档时可统一补「决策主档在 qx-map `__archive__/decisions/`」说明行。
- 「MCP selftest ALL PASSED」未单独复跑（kb MCP 服务接入是 T37 范围）；本卡检索能力经 search.py 独立实测确认。
