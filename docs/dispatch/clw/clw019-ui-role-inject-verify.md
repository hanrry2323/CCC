...
**范围与真实性（核对通过）**：本卡为只读审查卡，改动仅限卡文件 + 教训笔记 `docs/notes/2026-08-11-clw-cross-node-worker-lessons.md`（确认存在且内容与卡一致）；未改 clwarp 源码，无 `## 验收区`，卡未置关闭。分支 `codex/clw019-ui-role-inject-verify` 与卡命名一致。
