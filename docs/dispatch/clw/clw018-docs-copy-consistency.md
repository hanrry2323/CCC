# 任务卡 clw018 · 文档/文案一致性（去 GPU 字样 / i18n 补全 / RELEASE 对齐）（OpenCode 执行）
...
文档/文案一致性（去 GPU 字样 / i18n 补全 / RELEASE 对齐）（ccc-plan 切片）。
...
2. formatDate 传 zh-CN；index.html lang=zh-CN
3. RELEASE.md 指向 v0.2.0 交付报告（现指向 v0.1.0 且声称 Metal GPU）；CHANGELOG 与实现一致
4. 回归：tsc -b && vite build 通过
