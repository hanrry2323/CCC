# 任务卡 clw023 · codex 可执行性修复（P0 确定根因：codex not found → 会话打不开）（OpenCode 执行）
...
codex 可执行性修复（P0 确定根因：codex not found → 会话打不开）（ccc-plan 切片）。
...
2. build_provider_command 对 codex 加可执行性预检：resolve_executable_path 失败 → 返回明确错误，前端显示友好文案，不卡 loading
3. 前端 getFriendlyError 覆盖 command not found 场景（中文提示）
4. 回归：cargo build --release + tsc -b && vite build 通过
