...
> 关联：mx-plan-008 · 执行体：OpenCode · 验收：OpenCode · 状态：已关闭· 派发：engine · 项目：mx · 日期：2026-08-19
...
2. **教训沉淀**：本卡是否产出可复用教训？[有]（有 → 业务仓 lessons.md 或 CCC docs/notes/YYYY-MM-DD-<prefix>-lessons.md 新增一条）
   - 说明：教训已沉淀至业务仓 docs/lessons.md（2026-08-19 条目「tarpaulin --lib 仅跑内联测试会显著低估覆盖率」，随分支 codex/mx053-task 合入）：tarpaulin --lib 仅跑内联测试会显著低估覆盖率（78.59% vs 83.60%），CI 应不加 --lib。
...
> 结论：通过
> 来源：engine 自动落盘（engine-audit）· 2026-08-20 00:47
> 证据：机审 v4 指令（三级 · 必须遵循） 1. 对抗式找茬：假设有 P0/P1，找具体可复现问题；0 发现须给风险论证。 2. 三级判定 severity：影响面/改动深度/红线邻近各 1-3 分，合计 3-4=轻 5-7=中 8-9=重，任一维度高→强制重。 3. 可快速修复的轻问题 → 就地修复并 commit+push（不打回）；原则性红线（业务意图违背/系统性越界）→ 打回。 4. 结论行必须输出 severity 标记（severity：轻/中/重）并明示结论（通过 / 不通过，不通过须附原因）。 [ccc.engine] child_pid=47579 [claude-code:unrecognized_model] {"model":"flash[1m]","query_source":"sdk"} 机审完成。 ## 审查摘要 | 维度 | 结论 | |------|-----
