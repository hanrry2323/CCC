...
PlaybackService 的 RateLimitMap 改为经构造注入，职责清晰、锁竞争收敛（不再强耦合内存单例）。

## 实现

- 背景：PlaybackService 强耦合内存 RateLimitMap（职责不清、锁竞争，mx025 审计 P2）。
- 要求：RateLimitMap 经构造注入，PlaybackService 不再强耦合内存单例，可替换可测试。
...
1. PlaybackService 的 RateLimitMap 经注入（可替换）。
2. PlaybackService 单测通过。
3. `cargo test` 相关测试全绿。
