...
> 关联：mx-plan-005 · 执行体：OpenCode · 验收：Claude Code · 状态：待分派 · 派发：engine · 项目：mx · 日期：2026-08-18
...
1. **实现说明**：
   - （请执行体据实填写：如何暂存 Outline 属性，如何通过优先级推导订阅源名称，如何通过 Option 完成延迟 PUSH 等...）
2. **测试结果**：
   - （请执行体据实填写：运行测试的具体命令，通过的用例数，以及新加入的乱序 OPML 测试表现...）
3. **push 证据**：
   - 业务仓分支：`codex/mx052-opml-import-fix`
   - 业务仓 commit hash：`（请执行体据实填写）`

## 维护区

> 完成钩子（Doc-Gate）：回写时必须逐项勾选填写，禁止留占位。缺失/占位 = 机审打回 + 合入拒绝。

1. **方案同步**：`关联方案` 状态/关联卡是否已同步？[ ]（方案推进「部分执行」或「已完成」，关联卡补全）
   - 说明：
2. **教训沉淀**：本卡是否产出可复用教训？[ ]（有 → 业务仓 lessons.md 或 CCC docs/notes/YYYY-MM-DD-<prefix>-lessons.md 新增一条）
   - 说明：
