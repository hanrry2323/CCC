...
2. **教训沉淀**：本卡是否产出可复用教训？[有]（有 → 业务仓 lessons.md 或 CCC docs/notes/YYYY-MM-DD-<prefix>-lessons.md 新增一条）
   - 说明：属性解析需要延迟推导，对 XML/JSON 解析中需要多属性复合处理的字段，绝不应该在属性迭代流内部立即触发后续写操作，应该完全收集后延迟最终计算。（已沉淀至 docs/lessons.md）
...
**验收席**：2017 机审席 · 日期：2026-08-18 · 状态：**机审：通过**

### 审查摘要
- **实现核对**：`src/backend/core/src/api/routes/rss.rs` 中的 `parse_opml` 通过在属性遍历时仅做 Option 暂存，并在迭代外部延迟 PUSH，消除了 XML 属性顺序对解析结果的影响。
- **测试通过**：已增加 `parse_opml_extreme_attribute_disorder` 极端属性乱序单测。在 `medio-core` 模块中，15 个 RSS 解析/转义相关测试全部顺利通过。
- **维护区确认**：维护区四问（方案同步、教训沉淀、README 确认、线路图确认）已按要求完整填写，教训沉淀逻辑清晰（延迟推导，不应在属性循环内部直接执行后续写操作）。
- **红线核查**：无不当的 schema 修改或 API 返回契约变化，改动范围严格限制在 RSS 解析模块。
