# 任务卡 mx030 · 安全 P1 修复（XSS/鉴权白名单/暴力破解/SSRF）（OpenCode 执行）

> 关联：mx-plan-002
> 执行 cwd：Engine 派发注入业务仓 worktree（禁止主仓目录） · 执行体：OpenCode · 验收：OpenCode · 状态：待分派（重投 2026-08-12） · 派发：engine · 项目：mx · 日期：2026-08-12
...
安全 P1 修复（XSS/鉴权白名单/暴力破解/SSRF）（ccc-plan 切片）。
...
1. RSS 渲染移除 dangerouslySetInnerHTML（改白名单过滤组件），script 标签注入用例通过前端测试
2. POST 鉴权白名单 fail-closed：未注册端点默认拒绝，新增端点注册集中管理，测试覆盖
3. 认证端点有暴力破解防护（速率限制/退避/锁定），pytest 覆盖连续失败场景
4. 远程图片下载增加超时、重试与大小上限，SSRF 防护用例通过
5. issues.jsonl 中 4 个 P1 项更新为 closed，全仓回归（cargo test + vitest + pytest）绿
