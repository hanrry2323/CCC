# 任务卡 xy026 · 测试门禁修复与文档除债（OpenCode 执行）

> 关联：测试门禁修复与文档除债（重开） · 执行体：OpenCode · 验收：OpenCode · 状态：待分派 · 派发：engine · 项目：xy · 日期：2026-08-08
...
升级后重开（2026-08-08）：原卡打回后清理重派，机制缺陷已修复（50x 基础设施误判、回写凭证强校验均放宽）。
执行体先读本批注并按原卡目标执行；回写区只需如实填写实现说明/测试结果/push 证据（commit hash），无需特定格式凭证行。
