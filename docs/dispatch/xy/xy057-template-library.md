...
- **依赖链（2026-08-20 定稿，见方案转卡计划）**：本卡无代码级依赖；方案定序 **M6 全部卡验收后启动**（M5 承接 M6）——卡头「依赖」列为 M6 全部四卡（xy052/xy053/xy054/xy055），engine 按依赖自动排程。同仓 max_concurrent=1，与 xy056 顺序执行。xy058 依赖本卡合入。
...
- 关联方案摘要（xy-plan-008 M5 视频高表现力二期·功能卡 5.2 模板库规模化 + html-preview）：模板扩充 + 场景预览命令。验收：模板库 ≥6 套（现有 3+新 3），preview 命令对任意已生成场景可用。依赖：无；M6 全部验收后启动（方案定序）。
