...
> **2026-08-15 架构定论**：知识库底座固化（M1）已收口。确立六条主里程碑——**M2 稳控与可恢复**（hp-plan-004：源码回灌 SSOT/双仓归一/全文摄入主备改造/凭据治理/可重建）、**M3 可观测与告警**（hp-plan-005：健康三态/告警推送）、**M4 数据保鲜与质量**（hp-plan-006：重灌/短 chunk/相关性）、**M5 生态消费**（hp-plan-007：mx/qb/xy 接入+流程联动）、**M6 演进**（多模态图谱 K12/高维向量/高可用，内容待定）。M2-M5 方案已落库待排期。
