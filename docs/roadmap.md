...
> **下一程意向（0.2.0 挂账 · clw-plan-002）**：v0.1.0 实测暴露核心链路不可用（GUI PATH 导致会话拉不起、终端无 resize/退出检测、dev 端口不匹配）且声明大于实际（GPU/Metal、看板内嵌、设置面板均未真实落地）。0.2.0 全量重构：P0 执行链修复（clw008）→ 终端链路重做（clw009）→ 前端 UI 重建（clw010）→ 看板+设置兑现（clw011）→ 工程化基座（clw012）。
> **远期（0.3.0+）**：Linux (Ubuntu/AppImage) 多端适配与远程会话中继（WebRTC/SSE）集成。
