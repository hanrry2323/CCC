...
> **⚠️ 过期标注 (2026-07-07)**：
> 本文件描述的"v0.5.0"是早期定位时的版本号。实际工程进度：
> - **v0.6 调度器集成阶段**:已完成,代码在 git history(v0.6-ide-scheduler.phases.json 收录)
> - **v0.7 阶段(v0.7-slim → v0.7f 共 9 子任务)**:全部 PASS,统一收束为 **v0.7.0** (2026-07-07)
> - **当前最新版本:v0.7.0**(见 `CHANGELOG.md`)
> - **v0.8 路线**:未启动,等用户拍板(`.ccc/state.md` "当前任务" 段)
>
> 本文件保留作为 v0.5 时期路线设计的历史存档。**新任务参考 `.ccc/state.md`**。
