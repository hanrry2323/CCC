...
| 面 | 摘要 | 自检结果 | 完成 |
|----|------|----------|------|
| 壳 | A bold/heading 字重；B toolResult 不再置 toolsFinished；C 去 count 动画 + prior-steps id；D resultHint；E 3pt 分段轨；F 首条顶部 Spacer；G 遮罩期内钉底 + 无动画恢复 opacity。ToolStep 在 ToolProgressRail.swift（非 Models）加 resultHint，Codable 兼容 | `swift build` 绿；`ccc-self-check.sh` 全通过 | ✅ |
| 过桥 | — | — | 不参与 |
| 编排 | — | — | 不参与 |
...
|----|------|
| 结论 | 通过 |
| 缺口 | E 的「未达段」未实现（steps 增量到达，无未来占位段）；done/total 已由 accessibility label 表达，非阻塞 |
| 验收日 | 2026-07-21 |

验收照单勾：
- [x] A Markdown：bold → semibold；heading → bold/semibold/medium（`MarkdownText.swift`）
- [x] B 绿勾：`.toolResult` 分支删 `if allDone { toolsFinished = true }`；只 `.done` 置 true（`AppModel.swift`）
- [x] C 闪烁：移除 `.animation(value: steps.count)`；prior-steps DisclosureGroup 加稳定 id；stepList `id: \.id`
- [x] D 摘要：`ToolStep.resultHint` + `ToolProgressHelper.resultHint(name:ok:label:)`；rail 顶部 + 展开均显示
- [x] E 进度轨：`segmentProgressTrack` 3pt 分段（done/running+mini ProgressView/error）
- [x] F 首条位置：`count==1 && user && !streaming` 顶部 Spacer `max(h*0.28, 140)`
- [x] G 切换漂移：遮罩期内 `bottomPinTick &+= 1` 钉底 + 预置 lastScrollTargetId；恢复用 `disablesAnimations` 直跳 1
- [x] `bash scripts/ccc-self-check.sh` → 全通过
- [x] `cd desktop && swift build` → Build complete（壳窗自检）
- [x] 白名单外无改动（4 desktop 文件 + brief；Models.swift 未动 —— ToolStep 实际定义在 ToolProgressRail.swift，resultHint 加在该处，正确）
- [x] 边界基线未破
