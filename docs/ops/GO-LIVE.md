...
> 🔴 **已退役（2026-08-02 重构）**：本文描述旧 Hub 时代架构（Desktop 主入口 + Hub `:17777`/`:7777` + Board `:7775`），部署已全部重建。现行架构为 **2017 单端 `:7788`** 生产 + M1 中枢；**禁止据此操作**。现行拓扑见 [`../deploy/topology.md`](../deploy/topology.md)；**（原文保留，仅供历史追溯）**
