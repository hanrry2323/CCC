...
> 🔴 **已退役（2026-08-02 重构）**：本文描述旧 Hub 双机架构（Desktop 主入口 + Hub `:7777` / sidecar `:7788` / 隧道 `:17777`），部署已全部重建。现行架构为 **2017 单端 `:7788`** 生产 + M1 中枢；**禁止据此操作**。现行拓扑见 [`../deploy/topology.md`](../deploy/topology.md)；**（原文保留，仅供历史追溯）**
...
3. 选业务项目 → 本机对话聊定意图 → **Agent 自动投意图链**（需 Hub 投递）→ 右栏看意图卡链 + 看板计数
   （快捷仅「对齐基线」「扫风险」；**无**「转意图卡」按钮）
