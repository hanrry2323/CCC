...
- 卡头「项目」**必须** = 文件名前缀 = 子目录名。  
- **卡头字段定死，禁止自创**：卡头只有模板里的字段（关联/执行体/验收/状态/派发/项目/日期/依赖），禁止加 `批准`/`审批`/`review` 等自造字段。「合入批准」是人审动作（流程），不是卡头字段。
- **依赖链 = 代码级技术依赖，不是流程排序**：只有「本卡代码必须用另一张卡的产物才能编译/运行」时才写依赖。方案定序（M6 先于 M5）是里程碑规划，不是代码依赖——卡可并行开发，合入时各自验证。写错依赖 = 阻塞并行 = 流程变慢。
