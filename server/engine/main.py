...
# 解环（2026-09-05）：infra 冷却/挂起归位 failure_class（infra 域）；此处 re-export 兼容旧引用。
from server.engine.failure_class import _hold_infra_failure  # noqa: E402
# R2 第一刀（2026-09-05）：worktree 域抽至 server/engine/worktree.py，此处全量 re-export
# 保持外部引用路径不变（web/worktree_dirty、tests、脚本均经 main 引用）。
from server.engine.worktree import (  # noqa: E402,F401
    _GIT_DEFAULT_TIMEOUT,
    _SEED_HARDFAIL_MARKER,
    _WORKTREE_FAILURES_LOCK,
    _bump_worktree_failures,
    reset_worktree_failure_count,
    worktree_failure_count,
    _business_worktree_path,
    _card_rel_path_in_worktree,
    _cleanup_business_worktrees,
    _cleanup_closed_worktrees,
    _commit_and_push_worktree_card,
    _ensure_business_worktree,
    _ensure_worktree_card_seed,
    _local_main_has_card,
    _mount_business_worktree_venv,
    _remote_branch_audit_evidence,
    _remove_business_worktree_venv,
    _seed_hardfail_alert,
    _self_heal_worktree_card,
    _worktree_branch_seed,
    _worktree_branch_tip,
    _worktree_card_candidate,
    _worktree_has_new_commit,
    _worktree_has_nonempty_diff,
    _worktree_hint_for,
    _worktree_root_from,
    get_worktree_path,
)
# R2 结果契约纯工具抽出；main 保留 re-export 兼容旧调用。
...
# 业务仓 worktree 失败计数已归 worktree.py；main 只通过 worktree_failure_count/reset_worktree_failure_count 读写。
...
def run_once(
    ...
    *,
    ...
    worktrees_failed = worktree_failure_count()
    reset_worktree_failure_count()
...
# ── 公共 API 别名（2026-09-05 深扫解环：web 层经此引用，不再 import 下划线私有）──
card_machine_audit_passed = _card_machine_audit_passed
run_machine_audit_after_writeback = _run_machine_audit_after_writeback
audit_marker_alive = _audit_marker_alive
