...
def _get_current_state(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    git_commit = ''
    try:
        res = subprocess.run(['git', 'log', 'origin/main', '--merges', '-n', '1', '--format=%H'], capture_output=True, text=True, timeout=5)
        if res.returncode == 0:
    ...
    if not git_commit:
        try:
            res = subprocess.run(['git', 'log', '--merges', '-n', '1', '--format=%H'], capture_output=True, text=True, timeout=5)
            if res.returncode == 0:
                git_commit = res.stdout.strip()
        except Exception:
            pass
    ...
    try:
        ...
        pass
    return {'timestamp': now, 'git_commit': git_commit, 'cards_index_mtime': cards_mtime, 'cards_index_size': cards_size}
        ...
        for p in projects_list:
            if p.taskable and p.prefix:
                ...
                if not re.search(pattern, roadmap_content):
                    findings.append({'id': f'missing_roadmap_section_{prefix}', 'title': f'项目 {prefix} 缺席 roadmap.md 的业务线路段落', 'project': prefix, 'type': 'missing_section', 'cross_confirm': 0.5, 'acting_on': 'docs/roadmap.md', 'evidence': 'docs/roadmap.md:1'})
                ...
                if normalize_state(r_status) != normalize_state(real_status):
                    ...
                    for idx, line in enumerate(roadmap_lines):
                        if f'**{card_id_raw}**' in line:
                            line_no = idx + 1
                            break
                    findings.append({'id': f'status_drift_{card_id_key}', 'title': f'任务卡 {card_id_raw} 状态漂移：roadmap.md 标注「{r_status}」，但看板/卡文件实际状态为「{real_status}」', 'project': card.get('project', 'ccc'), 'type': 'drift', 'cross_confirm': 0.5, 'acting_on': 'docs/roadmap.md', 'evidence': f'docs/roadmap.md:{line_no}'})
        ...
        if plan_status == '已完成':
            ...
            if cards_field:
                ...
                if open_ref_cards:
                    ...
                    findings.append({'id': f"completed_plan_open_cards_{plan['id']}", 'title': f"方案 {plan['id']} 已完成，但其关联卡未全部关闭: {open_cards_str}", 'project': plan.get('project', 'ccc'), 'type': 'broken_link', 'cross_confirm': 1.0, 'acting_on': plan.get('path', ''), 'evidence': f"{plan.get('path', '')}:1"})
    for plan in plans_list:
        cards_field = plan.get('cards', '')
        if cards_field:
            ref_cards = re.findall('([a-zA-Z]+[0-9]+)', cards_field)
            ...
            if missing_cards:
                findings.append({'id': f"plan_ref_missing_cards_{plan['id']}", 'title': f"方案 {plan['id']} 关联了不存在的任务卡: {', '.join(missing_cards)}", 'project': plan.get('project', 'ccc'), 'type': 'broken_link', 'cross_confirm': 0.5, 'acting_on': plan.get('path', ''), 'evidence': f"{plan.get('path', '')}:1"})
    for c in cards_list:
        state = c.get('state', '')
        if state in ('已关闭', '已合入', 'closed', 'released'):
            ...
            if relative_path:
                ...
                if missing:
                    findings.append({'id': f'missing_four_questions_{card_id.lower()}', 'title': f'已关闭任务卡 {card_id} 缺失或未完成维护区四问: {reason}', 'project': c.get('project', 'ccc'), 'type': 'missing_four_questions', 'cross_confirm': 0.5, 'acting_on': relative_path, 'evidence': f'{relative_path}:1'})
    return findings
...
def run_observer(cfg: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
    ...
    summary = {'timestamp': current_state['timestamp'], 'collected_at': datetime.datetime.fromtimestamp(current_state['timestamp']).isoformat(), 'projects_count': len(projects_list), 'cards_count': len(cards_list), 'plans_count': len(plans), 'cards_states': cards_states, 'plans_states': plans_states, 'projects': projects_list, 'findings': scored_findings}
    data_dir = cfg.get('DATA_DIR', '')
    if not data_dir:
        data_dir = os.environ.get('CCC_DATA_DIR') or os.environ.get('DATA_DIR') or 'data'
            ...
            if not re.match('^[a-zA-Z0-9]+-plan-\\d+$', token):
                ...
                for idx, line in enumerate(content.splitlines(), 1):
                    if '关联：' in line or '关联:' in line:
                        line_idx = idx
                        break
                findings.append({'type': 'governance', 'assertion': 2, 'acting_on': card_id, 'severity': 'YELLOW', 'msg': f"卡 {card_id} 的关联字段 '{related}' 不是有效的方案编号 (形如 <prefix>-plan-<NNN>)。", 'evidence': f'{rel_path}:{line_idx}', 'cross_confirm': 0.0})
        c_state = base_state(meta.get('状态', '未知'))
        if c_state in ('ignore', '已关闭', '已回写'):
            if '## 维护区' not in content:
                findings.append({'type': 'governance', 'assertion': 6, 'acting_on': card_id, 'severity': 'YELLOW', 'msg': f'卡 {card_id} 处于已关闭/已回写状态，但缺失 ## 维护区 章节。', 'evidence': f'{rel_path}:1', 'cross_confirm': 0.0})
            else:
                lines = content.splitlines()
                ...
                for idx, line in enumerate(lines, 1):
                    if '[是/否]' in line or '[有/无]' in line:
                        ...
                        break
                if has_placeholder:
                    findings.append({'type': 'governance', 'assertion': 6, 'acting_on': card_id, 'severity': 'YELLOW', 'msg': f'卡 {card_id} 的 维护区 包含未作答占位符 [是/否] 或 [有/无]。', 'evidence': f'{rel_path}:{placeholder_line}', 'cross_confirm': 0.0})
    if roadmap_path.exists():
        roadmap_content = roadmap_path.read_text(encoding='utf-8')
        row_pattern = re.compile('^\\s*\\|\\s*\\*\\*([a-zA-Z0-9_-]+)\\*\\*\\s*\\|\\s*([^|]+)\\s*\\|\\s*([^|]+)\\s*\\|', re.MULTILINE)
            ...
            if match:
                ...
                if expected_state:
                    ...
                    if actual_state and actual_state != expected_state:
                        findings.append({'type': 'governance', 'assertion': 5, 'acting_on': ref_card_id, 'severity': 'YELLOW', 'msg': f"路线图状态 '{status_cell}' (期望 {expected_state}) 与卡真实状态 '{actual_state}' 漂移。", 'evidence': f'docs/roadmap.md:{idx}', 'cross_confirm': 0.0})
    for plan in plans:
        plan_id = plan['id']
        ...
        if plan_status == '已完成' and (not associated_cards):
            findings.append({'type': 'reverse', 'assertion': 3, 'acting_on': plan_id, 'severity': 'YELLOW', 'msg': f'方案 {plan_id} 处于已完成状态，但没有关联任何开发卡。', 'evidence': f'{plan_path}:1', 'cross_confirm': 0.0})
        for ac in associated_cards:
            if ac not in card_status:
                findings.append({'type': 'governance', 'assertion': 4, 'acting_on': plan_id, 'severity': 'YELLOW', 'msg': f'方案 {plan_id} 引用了不存在的卡 {ac}。', 'evidence': f'{plan_path}:1', 'cross_confirm': 0.0})
        if plan_status == '已完成' and associated_cards:
            ...
            if unclosed_cards:
                findings.append({'type': 'governance', 'assertion': 3, 'acting_on': plan_id, 'severity': 'YELLOW', 'msg': f"方案 {plan_id} 已推进至 '已完成'，但其关联卡 {', '.join(unclosed_cards)} 尚未关闭。", 'evidence': f'{plan_path}:1', 'cross_confirm': 0.0})
        if plan_status != '已完成' and associated_cards:
            all_closed = True
            for ac in associated_cards:
                if card_status.get(ac) != '已关闭':
                    ...
                    break
            if all_closed:
                findings.append({'type': 'governance', 'assertion': 3, 'acting_on': plan_id, 'severity': 'YELLOW', 'msg': f"方案 {plan_id} 关联卡已全部关闭，但方案状态仍为 '{plan_status}' (未推进至 '已完成')。", 'evidence': f'{plan_path}:1', 'cross_confirm': 0.0})
                findings.append({'type': 'reverse', 'assertion': 3, 'acting_on': plan_id, 'severity': 'YELLOW', 'msg': f"从已关闭卡反推，方案 {plan_id} 应该已完成，但实际状态仍处于 '{plan_status}'。", 'evidence': f'{plan_path}:1', 'cross_confirm': 0.0})
        for ac in associated_cards:
            if card_status.get(ac) == '已关闭' and plan_status != '已完成':
                findings.append({'type': 'reverse', 'assertion': 7, 'acting_on': ac, 'severity': 'YELLOW', 'msg': f"开发卡 {ac} 已关闭，但其关联方案 {plan_id} 的状态仍为 '{plan_status}' (未完成)。", 'evidence': f'docs/dispatch/{ac}.md:1' if (dispatch_dir / f'{ac}.md').exists() else f'{plan_path}:1', 'cross_confirm': 0.0})
...
def gather_mcp_metrics(log_dir: Path) -> dict[str, Any]:
    ...
    return {'opencode_mcp_enabled': opencode_ok, 'claude_mcp_enabled': claude_ok, 'total_calls_observed': total_calls, 'call_success_rate': success_rate}
def gather_maintenance_metrics(dispatch_dir: Path) -> dict[str, Any]:
    ...
    return {'total_completed_cards': total_completed, 'complete_maintenance_cards': complete_maintenance, 'maintenance_coverage_pct': coverage}
def gather_lesson_recirculation_metrics(dispatch_dir: Path) -> dict[str, Any]:
    ...
    return {'total_new_cards': len(new_cards), 'recirculated_lessons_cards': recirculated, 'lesson_recirculation_rate_pct': recirculation_rate}
...
def run_playwright_smoke_test(url: str='http://127.0.0.1:7788') -> dict[str, Any]:
    ...
    try:
        ...
        return {'ok': False, 'status_str': '环境未就绪，待卡10后续 (未安装 Playwright 库)', 'health_status': '跳过', 'config_status': '跳过', 'main_status': '跳过'}
    try:
        with sync_playwright() as p:
            ...
            return {'ok': ok, 'status_str': status_str, 'health_status': '200 OK' if health_ok else '失败', 'config_status': '200 OK' if config_ok else '失败', 'main_status': '200 OK' if main_ok else '失败'}
    except Exception as e:
        return {'ok': False, 'status_str': f'环境未就绪/服务未运行 ({e})', 'health_status': '失败', 'config_status': '失败', 'main_status': '失败'}
